...
public interface BlockBakery {
    ...
    static float[] getTopAO(IWorld world, long x, long y, long z) {
        ...
        if (blockMPE.getBlock().isSolid()) {
            result[0] = 0.5f;
            result[1] = 0.5f;
        }

        if (blockPPE.getBlock().isSolid()) {
            result[2] = 0.5f;
            result[3] = 0.5f;
        }

        if (blockEPM.getBlock().isSolid()) {
            result[0] = 0.5f;
            result[2] = 0.5f;
        }

        if (blockEPP.getBlock().isSolid()) {
            result[1] = 0.5f;
            result[3] = 0.5f;
        }
    ...
    static float[] getAOModelModifier(IWorld world, long x, long y, long z) {
        ...
        if (blockEPM.getBlock().isSolid() && blockMPE.getBlock().isSolid()) {
            result[2] = 0.5f;
        }

        if (!(blockEPM.getBlock().isSolid() && blockMPE.getBlock().isSolid()) && blockMPM.getBlock().isSolid()) {
            result[2] = 0.5f;
        }


        if (blockEPP.getBlock().isSolid() && blockMPP.getBlock().isSolid()) {
            result[3] = 0.5f;
        }

        if (blockMPP.getBlock().isSolid()) {
            result[3] = 0.5f;
        }
    ...
    static double getSmoothedLight(IWorld world, long x, long y, long z, Vector3d relativePos) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            adjacentBlocks[0] = world.getBlockAccess(cornerX, y, cornerZ).getBlock().isSolid(); // 上下方向
            adjacentBlocks[1] = world.getBlockAccess(x, cornerY, cornerZ).getBlock().isSolid(); // 左右方向
            adjacentBlocks[2] = world.getBlockAccess(cornerX, cornerY, z).getBlock().isSolid(); // 前后方向
        ...
        /*
        if (blockEME.getBlock().isSolid() && blockEPE.getBlock().isSolid()) {
            return 0.5;
        }
        if (blockEME.getBlock().isSolid()) {
            return MathHelper.linearInterpolate(0.5, 1, relativePos.y);
        }

        if (blockEPE.getBlock().isSolid()) {
