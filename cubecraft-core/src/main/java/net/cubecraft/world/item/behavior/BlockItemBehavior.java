...
public class BlockItemBehavior extends DigableItemBehavior {
    ...
    public void onUse(HitResult result, Item item, BlockAccess block) {
        BlockAccess tgt = block.getNear(EnumFacing.fromId(result.getHitFace()));
        if (!((World) tgt.getWorld()).isFree(BlockPropertyDispatcher.getCollisionBox(Blocks.REGISTRY.object(item.getId()), tgt))) {
            return;
        }
